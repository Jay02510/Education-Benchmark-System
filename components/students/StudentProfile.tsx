...
export const StudentProfile: React.FC<StudentProfileProps> = ({ student, onBack }) => {
    ...
    const handleDeleteAssessment = async (id: string) => {
        if (window.confirm("Permanently delete this assessment record?")) {
            await deleteAssessmentForStudent(student.id, id);
        }
    };
            ...
            <div className="bg-white px-6 py-6 md:px-10 shrink-0 border-b border-slate-100 shadow-sm z-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-6">
                        <div className="relative group">
                            ...
                            <button onClick={() => setIsEditProfileModalOpen(true)} className="absolute -bottom-1 -right-1 p-1.5 bg-white border border-slate-100 rounded-xl shadow-lg text-slate-400 hover:text-indigo-600 transition-colors z-20"><Icon name="settings" className="w-3.5 h-3.5" /></button>
                        </div>
                        <div>
                            <div className="flex items-center gap-3">
                                <h1 className="text-3xl font-black text-slate-900 tracking-tight">{student.name}</h1>
                                <button onClick={() => setIsEditProfileModalOpen(true)} className="p-1.5 text-slate-300 hover:text-indigo-600 transition-colors"><Icon name="settings" className="w-5 h-5" /></button>
                            </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={() => { setAssessmentToEdit(null); setIsAssessmentModalOpen(true); }} className="px-5 py-2.5 bg-slate-900 text-white rounded-xl font-bold shadow-xl hover:bg-slate-800 active:scale-95 transition-all flex items-center gap-2">
                            <Icon name="plus" className="w-4 h-4" /> Log New Test
                        </button>
                    </div>
                </div>
                ...
                <div className="flex gap-6 mt-8 border-b border-slate-50">
                    {(['Overview', 'Assessments', 'Log', 'Resources'] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveSection(tab)} className={`pb-4 text-sm font-bold transition-all relative ${activeSection === tab ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                            {tab}{activeSection === tab && <div className="absolute bottom-0 left-0 w-full h-1 bg-indigo-600 rounded-full animate-in slide-in-from-bottom-2"></div>}
                        </button>
                    ))}
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                            <Card variant="glass" className="p-8 border-t-8 border-indigo-600 shadow-xl border-white/60">
                                <div className="flex justify-between items-center mb-6">
                                    <div><h3 className="font-black text-slate-800 tracking-tight">Growth Timeline</h3><p className="text-xs text-slate-400 font-medium">Visualization of outcome</p></div>
                                </div>
                                <div className="min-h-[300px]"><LongitudinalGrowthChart data={projectionData} lines={[{ key: 'score', color: '#6366f1' }]} type="area" actions={actionPoints} /></div>
                            </Card>
                ...
                {activeSection === 'Assessments' && (
                    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex justify-between items-end mb-4">
                            <div><h3 className="text-2xl font-black text-slate-900 tracking-tight">Assessment History</h3></div>
                            <button onClick={() => { setAssessmentToEdit(null); setIsAssessmentModalOpen(true); }} className="px-5 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-sm border border-indigo-100">Add Assessment</button>
                        </div>
                ...
                {activeSection === 'Log' && (
                    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <Card variant="paper" className="p-8 border-t-8 border-indigo-500 shadow-xl">
                            <h3 className="text-2xl font-black text-slate-900 mb-6 tracking-tight">Intervention Record</h3>
                            <div className="flex flex-col sm:flex-row gap-4 mb-8">
                                <select value={logCategory} onChange={(e) => setLogCategory(e.target.value as any)} className="px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                                    <option>Observation</option><option>Intervention</option><option>Parent Communication</option><option>Goal Met</option>
                                </select>
                                <div className="flex-1 flex gap-2">
                                    <input type="text" value={logText} onChange={(e) => setLogText(e.target.value)} placeholder="Add a progress note..." className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
                            ...
                            <div className="space-y-4">{student.actionLog?.map(entry => <LogEntryView key={entry.id} entry={entry} />)}</div>
                        </Card>
                    </div>
                )}
            </main>
