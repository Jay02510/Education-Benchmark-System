                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div>
                            <div className="flex items-center gap-4">
                                ...
                                {student.overallGrowth > 10 && <div className="px-3 py-1 bg-amber-50 text-amber-600 rounded-full border border-amber-100 text-[10px] font-black uppercase tracking-widest shadow-sm">Fast Track</div>}
                            </div>
                    ...
                    <div className="space-y-12 animate-in fade-in duration-700">
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                            <InsightCard 
                                ...
                                contextForAi={`Student ${student.name} at Level ${student.level} has ${student.growthVelocity}% velocity and ${currentProficiency}% overall proficiency.`}
                                actionLabel="Optimize Pathway"
                                onAction={() => setActiveSection('Resources')}
                            >
                                <div className="min-h-[350px]">
                                    <LongitudinalGrowthChart data={projectionData} lines={[{ key: 'score', color: '#4f46e5' }]} type="area" actions={actionPoints} />
                                </div>
                            </InsightCard>
                            ...
                            <InsightCard 
                                ...
                                contextForAi={`Student strengths are at ${currentProficiency}%. Analyze the domain balance.`}
                            >
                                <div className="min-h-[350px]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            <ProfileStatWidget title="Weighted Mastery" value={`${currentProficiency}%`} subtext="Standards Met" icon="analytics" gradient="from-blue-600 to-indigo-700" tooltip="Comprehensive mastery score across all tested subdomains." />
                            <ProfileStatWidget title="Growth Speed" value={`${student.growthVelocity}%`} subtext={student.growthVelocity >= 10 ? "Fast Track" : "Steady"} icon="trendUp" gradient={student.growthVelocity >= 10 ? "from-emerald-500 to-teal-600" : student.growthVelocity < 0 ? "from-rose-500 to-pink-600" : "from-indigo-400 to-blue-500"} tooltip="Improvement velocity since last baseline." />
                            <ProfileStatWidget title="Calibration" value="Starters" subtext="CEFR Alignment" icon="benchmark" gradient="from-purple-600 to-indigo-600" tooltip="International mapping based on target proficiency." />
                            <ProfileStatWidget title="Action Records" value={student.actionLog?.length || 0} subtext="Logged Insights" icon="chat" gradient="from-slate-800 to-slate-950" tooltip="Teaching interventions and observations." />
                        </div>
                    </div>
                )}
