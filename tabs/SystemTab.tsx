                ...
                <div className="lg:col-span-9">
                    <Card className="p-10 bg-white border-none shadow-2xl rounded-[3.5rem] h-full min-h-[700px] relative overflow-hidden">
                        ...
                        {activeSection === 'institutional' && (
                            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500 relative z-10">
                                <div className="flex justify-between items-start">
                                    ...
                                    <button 
                                        onClick={() => { if(window.confirm("WARNING: This will delete all custom domains and subdomains, reverting to Master standards. Proceed?")) resetBenchmarks(); }} 
                                ...
                                <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100">
                                    <div className="flex gap-4 mb-10">
                                        <input 
                                            ...
                                            placeholder="Add Primary Segment (e.g. Logic, Coding)" 
                                            className="flex-1 px-8 py-5 bg-white border-2 border-slate-200 rounded-2xl outline-none focus:border-indigo-600 font-black text-lg shadow-sm"
                                        />
                                        <button 
                                            onClick={handleAddDomain}
                                            className="px-10 py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition-all active:scale-95 border-b-4 border-slate-950"
                                        >
                                            Add Segment
                                        </button>
                                    </div>
                                            ...
                                            <div key={d} className={`bg-white border transition-all rounded-[2.5rem] overflow-hidden ${expandedDomain === d ? 'border-indigo-200 shadow-xl' : 'border-slate-100 hover:border-indigo-100'}`}>
                                                ...
                                                >
                                                    <div className="flex items-center gap-4">
                                                        <div className={`p-3 rounded-xl transition-colors ${expandedDomain === d ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600'}`}>
                                                            <Icon name="benchmark" className="w-5 h-5" />
                                                        </div>
                                                        <div>
                                                            <span className="font-black text-slate-800 text-lg uppercase tracking-tight">{d}</span>
                                                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{(subdomains[d] || []).length} Components</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="px-8 pb-8 pt-2 animate-in slide-in-from-top-2 duration-200 bg-slate-50/30">
                                                            ...
                                                            {(subdomains[d] || []).map((sub: SubdomainMetadata) => (
                                                                <div key={sub.name} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 group shadow-sm">
                                                                    <div className="flex flex-col">
                                                                        ...
                                                                        <span className="text-[9px] font-bold text-slate-400 uppercase">Limit: {sub.maxScore} pts</span>
                                                                    </div>
                                                                    <button onClick={() => deleteSubdomain(d, sub.name)} className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-rose-500 transition-all">
                                                        ...
                                                        <div className="flex gap-3 bg-white p-4 rounded-[1.8rem] border border-slate-100 shadow-inner">
                                                            <input 
                                                                ...
                                                                placeholder="Component Name (e.g. Fluency)"
                                                            ...
                                                            <div className="flex items-center gap-2 px-4 border-l border-slate-100">
                                                                ...
                                                                <input 
                                                                    type="number" value={newSubMax} onChange={e => setNewSubMax(parseInt(e.target.value))}
                                                                    className="w-12 bg-slate-50 rounded-lg p-1 text-xs font-black text-center"
                                                                />
                                                            </div>
                                                            <button 
                                                                onClick={() => handleAddSub(d)}
                                                                className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-indigo-700 active:scale-95 transition-all"
                                                            >
                                                                Attach
                                                            </button>
                                                        </div>
                        ...
                        {activeSection === 'security' && (
                            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500 relative z-10">
                                <div>
                                    <h2 className="text-3xl font-black text-slate-900 mb-2 tracking-tight">Intervention Sensitivity</h2>
                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Calibration of Automated RTI Trigger Logic</p>
                                </div>
                                <div className="space-y-12">
                                    {Object.values(TestPeriod).map((period) => (
                                        <div key={period} className="space-y-4 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100">
                                            ...
                                            />
                                            <div className="mt-4 flex items-center gap-3 p-4 bg-white rounded-2xl border border-slate-100">
                                                <Icon name="info" className="w-4 h-4 text-indigo-400" />
                                                <p className="text-[10px] text-slate-500 font-bold italic leading-relaxed uppercase tracking-wider">"Students below {thresholds[period]}% in {period} will trigger an active intervention alert."</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
